...
int main(void)
{
    //1: Bare-Metal Hardware Register Configuration.
    RCC_AHB1ENR |= (1 << 3);   //Enable clock gating for GPIOD,
    GPIOD_MODER |= (1 << 24); //GPIOD Pin 12 as general-purpose output,

    //2: Interrupt-driven, DMA-backed SPI1 initialization,
    spi1_dma_init();

    //3: RTOS synchronization primitive for ISR -> task handoff,
    xSpiDmaCompleteSemaphore = xSemaphoreCreateBinary();

    //4: Initialize deterministic RTOS multi-threaded tasks,
